    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-lg">
        ...
        <h1 className="text-3xl font-semibold text-center text-gray-800 mb-8">
          News, Search & Trends Dashboard for "{data.keyword}"
        </h1>
        ...
        <div>
          ...
          <div className="space-y-4">
            {data.trends && data.trends.interest_over_time && data.trends.interest_over_time.timeline_data.map((timeline, index) => (
              <div key={index} className="p-4 border border-gray-200 rounded-lg shadow-md hover:shadow-lg transition mb-4">
                <h3 className="text-lg font-medium text-gray-700">{timeline.date}</h3>
                <p className="text-sm text-gray-600">
                  Value: <span className="font-semibold">{timeline.values[0].value}</span>
                </p>
              </div>
            ))}
          </div>
